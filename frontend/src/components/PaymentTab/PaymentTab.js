...
export default class App extends React.Component {
    ...
    getP = async () => {
    try {
      let busID = localStorage.getItem('selectedBusId');
      const response = await axios.get(`http://localhost:8080/getBusPrice/${busID}`);
      const pricePerSeat = response.data.pricePerSeat;
      this.setState({ pricePerSeat });
    } catch (error) {
      console.log(error);
    }
  };
    ...
    handleSubmit = e => {
        ...
        this.form.reset()
         // call mongodb to fetch the price
      
        // // mongoose.fin
        // axios.post('http://localhost:8080/getBusPrice/',{'busId':BusID}).then(data=> alert("bus Price => "+data.data.price))
    }
    ...
    getSumTotal = () => {
        ...
        let tax = 150
        let seatArray = localStorage.getItem('reservedSeats')
        // let price = localStorage.getItem('pricePerSeat')
        if (seatArray) {
            let seaArr = JSON.parse(seatArray)
            ...
            return (<div>
                ...
                <p> {Number(this.state.pricePerSeat)* count}</p> <p> +{tax} </p > < p > {Number(this.state.pricePerSeat) * count + tax} </p>{' '} </div>
            )
        }
    }
